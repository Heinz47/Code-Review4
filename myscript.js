 var array = [
    ...
    var allbtn = document.getElementsByClassName("btn");
        ...
        function passParameter(i){

            for (let i = 0; i < array.length ; i = 5){ let sort=document.getElementsByClassName("sort");
                btns[i].addEventListener("click",shortDescripionInfo) = incase;

                
            var btns = shortDescripionInfo(0); 
            var btns = shortDescripionInfo(1);
            var btns = shortDescripionInfo(2);
            var btns = shortDescripionInfo(3);
            var btns = shortDescripionInfo(4);
            var btns = shortDescripionInfo(5);

             console.log(sort);
  ​
        function shortDescripionInfo(i){
             if(array[i].shortDescripionInfo(0)){ if(array[i].likes == number)
                 console.log(array[i].)
             }else if(array[i].shortDescripionInfo(0)){ if(array[i].likes == number)
                 console.log(array[i].)
             }else{array[i].shortDescripionInfo(0){ if(array[i].likes == number)}
                console.log("array[i]");
